...
const Signal<float, TIME_DOMAIN> window = { 1, 2, 3 };
...
TEST_CASE("Low pass windowed view", "[FIR Descs]") {
	const auto desc = Lowpass(WINDOWED).Cutoff(winCutoff).Window(window);
...
TEST_CASE("High pass windowed view", "[FIR Descs]") {
	const auto desc = Highpass(WINDOWED).Cutoff(winCutoff).Window(window);
	REQUIRE(desc.cutoff == winCutoff);
	REQUIRE(desc.window.Size() == 3);
	REQUIRE(desc.window[0] == 1);
}
...
TEST_CASE("Band pass windowed view", "[FIR Descs]") {
	const auto desc = Bandpass(WINDOWED).Band(winBandLow, winBandHigh).Window(window);
...
TEST_CASE("Band stop windowed view", "[FIR Descs]") {
	const auto desc = Bandstop(WINDOWED).Band(winBandLow, winBandHigh).Window(window);
	REQUIRE(desc.low == winBandLow);
	REQUIRE(desc.high == winBandHigh);
	REQUIRE(desc.window.Size() == 3);
	REQUIRE(desc.window[0] == 1);
...
TEST_CASE("Arbitrary windowed view", "[FIR Descs]") {
	const auto desc = Arbitrary(WINDOWED).Response([](float f) { return 1.f; }).Window(window);
