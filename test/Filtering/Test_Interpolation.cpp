...
TEST_CASE("Polyphase interpolation", "[Interpolation]") {
	...
	const Signal<float> filter = FirFilter<float, TIME_DOMAIN>(31, Lowpass(WINDOWED).Cutoff(1.0f / numFilters));
	Signal<float> scratch(filter.Size());
	const auto polyphase = PolyphaseNormalized(PolyphaseDecompose(scratch, filter, numFilters));

	const Signal<float> signal = MakeRamp(150);
	Signal<float> output(numFilters * ConvolutionLength(signal.Size(), polyphase.Size(), CONV_FULL));
...
TEST_CASE("Polyphase resampling replicate convolution full", "[Interpolation]") {
	...
	const Signal<float> signal = MakeRamp(150);
	Signal<float> padded(signal.Size() * numFilters);
	Expand(padded, signal, numFilters);

	Signal<float> outputConv = Convolution(padded, filter * numFilters, CONV_FULL);
	Signal<float> output(outputConv.Size());
...
TEST_CASE("Polyphase resampling upsample constant", "[Interpolation]") {
	...
	Signal<float> signal(150, 1.0f);
	Signal<float> output(signal.Size() - polyphase[0].Size() - 1);
...
TEST_CASE("Polyphase resampling downsample ramp mild", "[Interpolation]") {
	...
	const Signal<float> filter = FirFilter<float, TIME_DOMAIN>(31, Lowpass(WINDOWED).Cutoff(1.0f / ratioReal / numFilters));

	Signal<float> scratch(filter.Size());
	const auto polyphase = PolyphaseNormalized(PolyphaseDecompose(scratch, filter, numFilters));

	const Signal<float> signal = MakeRamp(150);
	Signal<float> output(signal.Size() * ratio.second / ratio.first - polyphase[0].Size() - 1);
...
TEST_CASE("Polyphase resampling shift ramp", "[Interpolation]") {
	...
	const Signal<float> filter = FirFilter<float, TIME_DOMAIN>(63, Lowpass(WINDOWED).Cutoff(1.0f / numFilters));
	Signal<float> scratch(filter.Size());
	const auto polyphase = PolyphaseNormalized(PolyphaseDecompose(scratch, filter, numFilters));
